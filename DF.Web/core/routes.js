(function () {
    ...
    function routeFn($routeProvider) {
        $routeProvider
            ...
            .when('/report-training-member-presence',
                {
                    controller: 'Report_TrainingMemberPresence_Controller',
                    templateUrl: 'pages/reports/training-member-presence/member-presence.html?nd=' + Date.now(),
                    resolve: {
                        danceGroups: function (DanceGroupsService, AuthenticationService) {
                            var currentUser = AuthenticationService.getCurrentUser();
